...
class GUI:
    ...
    def __init__(self, game_env):
        ...
        self.colors = {
            ...
            'background': '#feb825',
            'widget': '#8d4200',
            'thorn': '#ff0000',  # Color for thorn tiles
            'slip': '#0000ff'    # Color for slip tiles
        }
    ...
    def draw_environment(self):
        ...
        for i in range(self.game_env.n_rows):
            for j in range(self.game_env.n_cols):
                ...
                elif self.game_env.thorn_map[i][j]:  # Draw thorn tiles
                    x, y = self.hexagon_positions[(i, j)]
                    self.draw_hex(x, y, self.colors['thorn'])
                elif self.game_env.slip_map[i][j]:  # Draw slip tiles
                    x, y = self.hexagon_positions[(i, j)]
                    self.draw_hex(x, y, self.colors['slip'])
    ...
    def perform_action(self, action):
        reward, new_state = self.game_env.perform_action(self.state, action)
