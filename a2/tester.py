...
from state import State as ControlState
from environment import Environment as ControlEnvironment


"""
Tester script. Multiprocessing Version.

Use this script to evaluate your solution. For local testing you may modify this file as needed.
...
def state_stable_hash(s: ControlState):
    return stable_hash(str((s.BEE_posit, s.BEE_orient, s.widget_centres, s.widget_orients)))
...
def run_test_mp(filename_i_vis):
    ...
    for j in range(VALIDATION_SET_SIZE):
        ...
        for k in range(VALIDATION_SET_LOOKAHEAD):
            ...
            temp_action = random.choice(BEE_ACTIONS)
